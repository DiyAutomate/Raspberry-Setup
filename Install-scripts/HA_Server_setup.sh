...
sudo echo "[Unit]" > /etc/systemd/system/hass.service
sudo echo "Description=Home Assistant" >> /etc/systemd/system/hass.service
sudo echo "After=network.target" >> /etc/systemd/system/hass.service
sudo echo " " >> /etc/systemd/system/hass.service
sudo echo "[Service]" >> /etc/systemd/system/hass.service
sudo echo "Type=simple" >> /etc/systemd/system/hass.service
sudo echo "User=pi" >> /etc/systemd/system/hass.service
sudo echo "ExecStart=/usr/local/bin/hass" >> /etc/systemd/system/hass.service
sudo echo " ">> /etc/systemd/system/hass.service
sudo echo "[Install]" >> /etc/systemd/system/hass.service
sudo echo "WantedBy=multi-user.target" >> /etc/systemd/system/hass.service
