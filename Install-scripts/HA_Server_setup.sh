...
echo "installing rmate"
   #install rmate
sudo wget -O /usr/local/bin/rmate https://raw.github.com/aurora/rmate/master/rmate" >> .SystemUpdate.sh
sudo chmod a+x /usr/local/bin/rmate" >> .SystemUpdate.sh
...
echo 'installing git'" >> .SystemUpdate.sh 
  #install git
apt-get install git" >> .SystemUpdate.sh
...
sudo pip3 install sqlalchemy" >> .SystemUpdate.sh
sudo pip3 install --upgrade homeassistant" >> .SystemUpdate.sh

    # if service does not auto install uncomment these lines otherwise comment them out.
sudo touch /etc/systemd/system/hass.service" >> .SystemUpdate.sh
...
sudo echo "sudo systemctl --system daemon-reloadsudo echo "
